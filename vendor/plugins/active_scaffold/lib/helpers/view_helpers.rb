  ...
  module Helpers
    ...
    module ViewHelpers
      ...
      def link_to_visibility_toggle(options = {})
        ...
        link_text = options[:default_visible] ? 'ocultar' : 'ver'
        link_to_function as_(link_text), "e = #{options[:of]}; e.toggle(); this.innerHTML = (e.style.display == 'none') ? '#{as_('ver')}' : '#{as_('ocultar')}'", :class => 'visibility-toggle'
      end
