module ActiveScaffold::Config
  class Show < Base
    ...
    # --------------------------
    cattr_accessor :link
    @@link = ActiveScaffold::DataStructures::ActionLink.new('show', :label => 'Ver', :type => :record, :security_method => :show_authorized?)
    ...
    attr_writer :label
    def label
      @label ? as_(@label) : as_('Ver %s', @core.label.singularize)
    end
