    ...
    def include_habtm_actions
      if nested_habtm?
        ...
        if active_scaffold_config.nested.shallow_delete
          ...
          active_scaffold_config.action_links.add('destroy', :label => 'Eliminar', :type => :record, :confirm => 'Are you sure?', :method => :delete, :position => false, :security_method => :delete_existing_authorized?) unless active_scaffold_config.action_links['destroy']
        end
  ...
  module ChildMethods
    ...
    def add_existing
      ...
      respond_to do |type|
        type.html do
          if successful?
            flash[:info] = as_('Creado %s', @record.to_label)
            return_to_main
          else
            render(:action => 'add_existing_form', :layout => true)
          end
    ...
    def destroy_existing
      ...
      respond_to do |type|
        type.html do
          flash[:info] = as_('Eliminado %s', @record.to_label)
          return_to_main
        end
